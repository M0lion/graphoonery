...
- [x] Add `createSampler` / `destroySampler` (new `vulkan/src/sampler.zig`, or into
...
- [x] `DescriptorType` (`descriptor.zig:5`) — add
      `CombinedImageSampler = c.VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER`
- [x] `ShaderStage` (`descriptor.zig:8`) — add
      `Fragment = c.VK_SHADER_STAGE_FRAGMENT_BIT`
- [x] `createDescriptorPool` (`descriptor.zig:54`) — hardcodes one
      `UNIFORM_BUFFER` pool size; add a `COMBINED_IMAGE_SAMPLER` size
- [x] `updateDescriptorSet` (`descriptor.zig:110`) — writes `pBufferInfo` only;
...
- [x] `shaders/canvas.vert` — no vertex buffer, 4 verts, triangle strip (same
